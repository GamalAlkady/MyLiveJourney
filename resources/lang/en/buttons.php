<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Messages Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines contain messages for various
    | actions. You are free to modify these language lines according
    | to your application's requirements.
    |
    */

    'create_new'  => '<i class="fa fa-plus fa-fw" aria-hidden="true"></i> <span class="hidden-xs">Create New</span><span class="hidden-xs"> :name</span>',
    'delete'        => '<i class="fa fa-trash-o fa-fw" aria-hidden="true"></i>',
    'delete2'        => '<i class="fa fa-trash-o fa-fw" aria-hidden="true"></i> Delete',
    'show'          => '<i class="fa fa-eye fa-fw" aria-hidden="true"></i>',
    'edit'          => '<i class="fa fa-pencil fa-fw mx-2" aria-hidden="true"></i>',
    'edit_name'          => '<i class="fa fa-pencil fa-fw me" aria-hidden="true"></i> Edit :name',

    'details'       => '<i class="fa fa-eye fa-fw" aria-hidden="true"></i> <span class="hidden-xs">Details</span>',


    'back_to'       => '<i class="fas fa-arrow-left text-primary" style="margin-inline-end:5px;"></i><span class="hidden-sm hidden-xs">Back to </span><span class="hidden-xs">:name</span>',
    'restore'       => '<i class="fa fa-refresh fa-fw" aria-hidden="true"></i>  <span class="hidden-xs">Restore</span>',
    'cancel'         => '<i class="fa fa-fw fa-times" aria-hidden="true"></i> Cancel',
    'save'   => '<i class="fa fa-fw fa-save" aria-hidden="true"></i> Save',
    // User Buttons
    'create_user'  => '<i class="fa fa-plus fa-fw" aria-hidden="true"></i> <span class="hidden-xs">Create New</span><span class="hidden-xs"> User</span>',
    'restore_user' => '<i class="fa fa-refresh fa-fw" aria-hidden="true"></i>  <span class="hidden-xs">Restore</span><span class="hidden-xs"> User</span>',
    'delete_user'   => '<i class="fa fa-trash-o fa-fw" aria-hidden="true"></i>  <span class="hidden-xs">Delete</span><span class="hidden-xs"> User</span>',
    'edit_user'     => '<i class="fa fa-pencil fa-fw" aria-hidden="true"></i> <span class="hidden-xs">Edit</span><span class="hidden-xs"> User</span>',


];
